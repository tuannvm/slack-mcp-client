...
func (b *LLMMCPBridge) generateToolPrompt() string {
	...
	if _, hasCreateCanvas := b.availableTools["canvas_create"]; hasCreateCanvas {
		...
		promptBuilder.WriteString("  * replace: Replace entire canvas (no section_id) OR specific section (with section_id)\n")
		promptBuilder.WriteString("  * insert_at_end/insert_at_start: Add content at end/start of canvas\n")
