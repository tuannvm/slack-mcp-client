...
func (b *LLMMCPBridge) generateToolPrompt() string {
	...
	if _, hasCreateCanvas := b.availableTools["canvas_create"]; hasCreateCanvas {
		...
		promptBuilder.WriteString("- Use canvas_edit to modify existing canvases (replace, insert_at_end, or insert_at_start)\n")
		promptBuilder.WriteString("- CRITICAL: When editing a canvas, you MUST use the exact canvas_id returned by canvas_create (e.g., 'F0982B35MGS')\n")
		promptBuilder.WriteString("- The canvas_create response includes {\"canvas_id\": \"F...\", ...} - save and reuse this ID\n")
		promptBuilder.WriteString("- Canvas IDs start with 'F' followed by alphanumeric characters\n")
